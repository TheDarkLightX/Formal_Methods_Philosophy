...
$M$ is a function from memory slots in $I$ to qNS Boolean-algebra values
$B_{\mathrm{qNS}}$.
...
<strong>Standard reading.</strong>
For each memory slot $i$, $M_{t+1}(i)$ is the join of the replacement value
$A(i)$ under guard $G(i)$ and the old memory value $M_t(i)$ under the prime of
$G(i)$.
...
<strong>Standard reading.</strong>
The left side is the denotation of applying `WrapExp` to $x$, applying
`WrapExp` again, then applying `WrapLog`. Under the abstract law
$\ln(\exp(a))=a$, the resulting denotation is $\exp(x)$.
...
<strong>Standard reading.</strong>
A constructor plan is generated by the grammar above. `Var` denotes the base
variable, `WrapExp(plan)` denotes the result of applying the checked
exponential context constructor to the subplan, and `WrapLog(plan)` denotes the
result of applying the checked logarithm context constructor to the subplan.
...
<strong>Standard reading.</strong>
`promote iff qNS accepted, proof accepted, and obligation audit ok` means the
promotion status is `promote` exactly when those three recorded predicates all
hold for the constructor-plan candidate.
...
<strong>Standard reading.</strong>
$T^\star$ is an element of $\mathcal{T}_{\le d}$ that minimizes the displayed
ordered tuple: noisy training mean-squared error, holdout mean-squared error,
tree size, tree depth, and tree representation.
