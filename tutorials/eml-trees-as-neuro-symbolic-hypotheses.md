...
$q_{\mathrm{NS}}(y\mid x)$ is
$q_{\mathrm{N}}(y\mid x)\chi_{\mathrm{S}}(y,x)$ divided by $Z(x)$, where
$Z(x)=\sum_{y'}q_{\mathrm{N}}(y'\mid x)\chi_{\mathrm{S}}(y',x)$.
...
<strong>Standard reading.</strong>
$q_{\mathrm{NS}}(y\mid x)$ is proportional to
$q_{\mathrm{N}}(y\mid x)\exp(\beta s_{\mathrm{S}}(y,x))$.
...
$q_{\mathrm{N}}(T\mid D)\chi_{\mathrm{valid}}(T)\chi_{\mathrm{spec}}(T,D)$.
...
<strong>Standard reading.</strong>
$T_t$ is sampled from $q_{\mathrm{N}}(\cdot\mid D,\theta_t)$,
$v_t=\operatorname{Verify}_{\mathrm{S}}(D,T_t)$, and
$\theta_{t+1}=\operatorname{Update}(\theta_t,v_t)$.
...
<strong>Standard reading.</strong>
$G_{\mathrm{holdout}}$ is the set $\{0.25,0.75,1.5,3\}$.
...
<strong>Standard reading.</strong>
$\chi_{\mathrm{spec}}(T,D)=1$ if and only if $\operatorname{MSE}(T,f)\le\varepsilon$.
...
<strong>Standard reading.</strong>
If `emitStep? before = some (cert, after)`, then `after.size < before.size`.
...
<strong>Standard reading.</strong>
For each listed tree `tree`, `(emitChainFuel tree.compileExpr.size tree.compileExpr).2`
is the listed target expression, and applying `emitStep?` to that target yields
`none`.
...
<strong>Standard reading.</strong>
`normalize(expr)` is defined as `(emitChainFuel expr.size expr).2`.
...
<strong>Standard reading.</strong>
If `equivByNormalize?(a,b) = true`, then `eval(a) = eval(b)`.
