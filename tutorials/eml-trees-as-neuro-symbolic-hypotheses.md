...
<strong>Standard reading.</strong>
$T$ ranges over the smallest tree language generated by three clauses: the
variable $x$ is a tree, the constant $1$ is a tree, and if two objects are
trees then an EML node built from those two trees is also a tree.
...
<strong>Standard reading.</strong>
$\operatorname{eml}(a,b)$ denotes the exponential of $a$ minus the natural
logarithm of $b$.
...
<strong>Standard reading.</strong>
$G_{\mathrm{holdout}}$ denotes the finite set whose four elements are
$0.25$, $0.75$, $1.5$, and $3$.
...
<strong>Standard reading.</strong>
`WrapLog(WrapExp(WrapExp(Var)))` is a constructor plan. It first applies
`WrapExp` to `Var`, applies `WrapExp` again to that result, and then applies
`WrapLog`.
...
<strong>Standard reading.</strong>
A plan is accepted by the budget only if its depth is at most $3$, its
`WrapExp` count is at most $2$, and its `WrapLog` count is at most $1$.
...
<strong>Standard reading.</strong>
An `exp_argument_guard` obligation is discharged when the sidecar has an
interval entry for that source expression in `exp_arguments_checked` and the
interval stays within the configured exponential bound. A
`positive_log_argument` obligation is discharged when the sidecar has an
interval entry for that source expression in `log_arguments_checked` with
...
<strong>Standard reading.</strong>
A constructor-plan candidate is promoted only when the corresponding sidecar
formula has `qns_status = accept`, has `proof_status = accepted`, and has an
`ok` constructor-obligation audit row.
...
<strong>Standard reading.</strong>
If a constructor plan has outer constructor $\operatorname{WrapLog}$ and its
child has outer constructor $\operatorname{WrapExp}$ with subplan $T$, the
normalizer replaces that two-constructor prefix by $T$.
...
<strong>Standard reading.</strong>
All four fixture rows are changed by the enabled meaning-preserving rewrite
rule. The sum of their plan depths decreases from $12$ to $4$, the sum of their
`WrapExp` counts decreases from $7$ to $3$, and the sum of their `WrapLog`
counts decreases from $5$ to $1$. After the direct `Var` receipt is added, all
four normalized rows are promoted.
...
<strong>Standard reading.</strong>
If one rewrite step succeeds on expression `before` and returns certificate
`cert` together with expression `after`, then the size of `after` is strictly
smaller than the size of `before`.
...
<strong>Standard reading.</strong>
After running the rewrite chain from `before` with fuel equal to the starting
size of `before`, applying one more rewrite step to the resulting expression
returns `none`.
...
<strong>Standard reading.</strong>
For any raw EML tree `tree` and normal form `norm`, if the finite checker
`tree.certNorm` returns `some norm`, then running the compiled-expression
strategy with fuel equal to the size of `tree.compileExpr` returns
`norm.compileExpr`.
...
<strong>Standard reading.</strong>
`normalize(expr)` is defined as the expression reached by running the rewrite
chain from `expr` with fuel equal to the size of `expr`. The soundness theorem
states that `expr` and its normalized form have the same evaluation. The
stability theorem states that the normalized form has no further accepted
rewrite step.
...
<strong>Standard reading.</strong>
If the normalization-based equivalence checker returns true on expressions
`a` and `b`, then `a` and `b` have the same evaluation.
...
<strong>Standard reading.</strong>
This is an EML node whose left child is $x$ and whose right child is another
EML node. That right child has left child $\operatorname{eml}(x,x)$ and right
child $1$.
