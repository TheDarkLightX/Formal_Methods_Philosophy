...
<div class="fp-callout fp-callout-note">
  ...
  <ul>
    <li><strong>Text basis.</strong> This tutorial is based on the public TABA draft on the Tau site and the public arXiv abstract for <em>Guarded Successor</em>. In a few later sections, the notation has been lightly normalized so the formulas read cleanly on the page. In particular, this tutorial often writes Boolean-algebra meet and join as <code>∧</code> and <code>∨</code>; TABA itself usually writes Boolean-algebra meet and join as juxtaposition or <code>∩</code> and <code>∪</code>, reserving <code>∧</code> and <code>∨</code> for logical connectives.</li>
...
- <code>0</code> is the bottom element. In the region picture, it is the empty region.
- <code>1</code> is the top element. In the region picture, it is the whole Boolean space.
- $a \wedge b$ is the meet of <code>a</code> and <code>b</code>. In the region picture, it is their shared part, or intersection.
- $a \vee b$ is the join of <code>a</code> and <code>b</code>. In the region picture, it is their union.
- <code>a'</code> is read as “a prime.” It denotes the Boolean-algebra operation on <code>a</code> that, in the region picture, gives everything outside <code>a</code> within the whole Boolean space.
...
then <code>f</code> is not a real-valued function like <code>x^2+1</code>.
It is a Boolean term with one distinguished variable.
If a Boolean-algebra element is substituted for <code>x</code>, the result is another Boolean-algebra element.
In particular,
...
So the two special evaluations are not numerical endpoint samples.
They are the two cofactors obtained by forcing the Boolean switch <code>x</code> to the bottom element or the top element.
...
<figure class="fp-figure">
  <p class="fp-figure-title">Atomless means no terminal live piece</p>
  {% include diagrams/atomless-splitting.svg %}
  <figcaption class="fp-figure-caption">
    On the left, an atom is a smallest nonzero piece. On the right, the atomless case never bottoms out: every live piece can still be split into two smaller nonzero pieces.
  </figcaption>
</figure>
...
A concrete countable example behind much of this intuition is the countable Cantor algebra and its completion.
This tutorial only needs that example as motivation for why atomless splitting is the right mental model.
...
- For every index <code>i</code>, the side-condition <code>g_i(x) ≠ 0</code> also holds at that same value of <code>x</code>.

<strong>English reading.</strong>

There exists a Boolean-algebra value of <code>x</code> such that <code>f(x)=0</code> and every side-condition <code>g_i(x) ≠ 0</code> also holds.
...
<strong>Reading trap.</strong>
The quantifier <code>∃x</code> ranges over Boolean-algebra elements.
...
The question has changed from "is <code>g</code> nonzero somewhere in the whole algebra?" to "is <code>g</code> nonzero somewhere inside the family of values that already solve <code>f(x)=0</code>?"
...
- Start with the original sentence <code>φ₀</code>.
- At each stage, add one more sentence splitter.
- Push the old obligations through the finer partition generated by that new splitter.

<strong>English reading.</strong>

Each round asks the old sentence to survive one more cut.
The live branch picture becomes finer and finer until the branch geometry stops changing in a semantically relevant way.
...
- Add a fresh control bit <code>flag</code>.
- Use the two values of <code>flag</code> to separate a witness branch from a checking branch.
- Reformulate the sentence so that universal success over the guarded form is equivalent to the original existence of a legal output choice.

<strong>English reading.</strong>

The flag is a bookkeeping device that lets the logic test both sides of a choice without leaving the guarded universal format.
...
- The value of <code>p</code> at time-offset <code>n</code> is constrained to satisfy <code>φ</code>.

<strong>English reading.</strong>

The rule constrains the value of <code>p</code> at offset <code>n</code> by the condition <code>φ</code>.
...
- The chosen pair <code>(x,y)</code> must satisfy <code>ψ(x,y)</code>.
- If there exists some witness <code>t</code> such that both <code>φ(x,t)</code> and <code>ψ(x,t)</code> hold, then the chosen <code>y</code> must also satisfy <code>φ(x,y)</code>.

<strong>English reading.</strong>

- Choose <code>y</code> so that <code>ψ(x,y)</code> holds.
- If some witness satisfies both <code>φ</code> and <code>ψ</code>, then <code>y</code> must also satisfy <code>φ</code>.
...
- Apply the new rule on inputs where the new rule is realizable.
- Fall back to the old rule on inputs where the new rule is unrealizable.
- Preserve overlap on inputs where both rules are available.

<strong>English reading.</strong>

- Apply the new rule on inputs where it is realizable.
...
### 6. Compiled forgetting as the executable QE backend

The bounded research result sitting closest to TABA's elimination chapter is not a new theorem of Boolean algebra.
It is a backend lesson.

The readable eliminant in TABA is the right object for exposition.
But if the goal is repeated exact quantifier elimination on a finite executable carrier, a compiled forgetting backend is a better operational fit.
...
- The left side existentially removes the variable <code>x</code>.
- The right side replaces that existential by the disjunction of the two cofactors obtained by forcing <code>x</code> to true and false.

<strong>English reading.</strong>

To forget one Boolean variable exactly, keep both branches and then merge them by disjunction.

That identity is already implicit in the cofactor story.
The backend change is that the merge can happen inside a shared canonical representation instead of inside a growing handwritten formula.

The Tau-side experiments refine that lesson one step further.
On the checked existential propositional fragment, direct compiled forgetting beat Tau's default qelim path on most bounded cases.
But inside the compiled carrier, variable order became a second optimization problem.
An appearance-based order and a frequency-based order each won on part of the checked set.
So even after choosing a compiled carrier, there is still no reason to expect one simple order heuristic to dominate every fragment.

The bounded comparison that matters most is:

$$
\exists x_1 \cdots \exists x_n\;L_n \;\equiv\; p \vee q_1 \vee \cdots \vee q_n,
$$

for a shared ladder family $L_n$, while

$$
\exists x_1 \cdots \exists x_n\;M_n \;\equiv\; y_0 \vee y_1 \vee \cdots \vee y_{2^n-1},
$$

for a unique-mux family $M_n$.

<strong>Standard reading.</strong>

- In the ladder family, eliminating the quantified block leaves a residual formula whose size grows only linearly with <code>n</code>.
- In the unique-mux family, eliminating the quantified block leaves a residual formula with <code>2^n</code> distinct surviving outputs.

<strong>English reading.</strong>

Some families are expensive only because the search tree repeats the same subproblem many times.
Those are the families where compiled forgetting wins.
Other families are expensive because the final answer itself is large.
No exact backend can compress that away.

What a next move would look like:

- keep TABA's cofactor eliminant as the explanation layer,
- lower finite executable fragments into a shared decision representation,
- perform forgetting there by existential abstraction,
- attach parity checks between the semantic layer and the executable carrier.

Why it is plausible:
Bryant's ordered decision diagrams give a canonical exact carrier for finite Boolean functions, and Darwiche's DNNF line shows that forgetting can become cheap once the representation is compiled. So the likely executable upgrade over raw TABA-style elimination is not a different meaning, but a better carrier.

### 7. An explicit operator budget
...
### 8. A finite atomic executable rail under the richer semantics
...
- [Randal E. Bryant, <em>Graph-Based Algorithms for Boolean Function Manipulation</em>](https://www.cs.cmu.edu/~bryant/pubdir/ieeetc86.pdf).
- [Adnan Darwiche, <em>Compiling Knowledge into Decomposable Negation Normal Form</em>](https://www.ijcai.org/Proceedings/99-1/Papers/042.pdf).
- [Adnan Darwiche, Pierre Marquis, <em>A Knowledge Compilation Map</em>](https://arxiv.org/abs/1106.1819).
