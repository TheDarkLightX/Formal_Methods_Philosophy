...
def build_report():
    v83 = build_v83()
    old_ladder = {
        row["residual_region_count"]: row for row in v83["budget_ladder"]
    }
    ...
    ladder = []
    for residual_budget in range(0, len(NONTRIVIAL_SCORES)):
        best = None
        ...
        prior = old_ladder.get(residual_budget)
        ladder.append(
            {
                ...
                "schema_gain_over_v83": None
                if prior is None
                else prior["optimal_shared_schema_count"] - best[0],
                "cost_gain_over_v83": None
                if prior is None
                else prior["optimal_total_cost"] - best[1],
            }
        )

    zero_budget = ladder[0]
    return {
        "tier": "descriptive_oracle",
        "oracle_dependent": True,
        "discovery_domain": (
            "joint search over score partitions and exact residual-region budgets "
            "on the hard v38 feature frontier, keeping residual-default witnesses "
            "in the 1-to-4 literal grammar while widening strict certificates to "
            "the 1-to-5 literal grammar"
        ),
        "holdout_domain": "the same 13 holdout fact states used from v29 through v84",
        "survivor": "hard widened-certificate partition-aware residual-budget frontier",
        ...
        "zero_residual_partition_exists": zero_budget["feasible_configuration_count"] > 0,
        "strongest_claim": (
            "On the hard refill witness frontier, widening the strict certificate side "
            "from the 1-to-4 literal conjunction grammar to the 1-to-5 literal grammar "
            "changes the full partition-aware residual-budget search, not only the local "
            "critical-region slice. The new joint frontier should be read off the exact "
            "budget ladder in this report, including whether a zero-residual all-positive "
