...
def build_report():
    ...
    prior = {
        ...
        if row["residual_region_count"] in HIGH_RESIDUAL_BUDGETS
    }
    ...
    ladder = []
    moved_budgets = []
    for residual_budget in HIGH_RESIDUAL_BUDGETS:
    ...
    return {
        ...
        "discovery_domain": (
            "high-residual slice of the hard partition-aware residual-budget frontier, "
        ...
        "holdout_domain": "the same 13 holdout fact states used from v29 through v85",
        "survivor": "high-residual widened-certificate saturation boundary",
        "feature_count": len(FEATURES),
        "feature_names": [feature_name(feature) for feature in FEATURES],
        "high_residual_budgets": list(HIGH_RESIDUAL_BUDGETS),
        ...
        "strongest_claim": (
            ...
            "does not move the high-residual end of the partition-aware residual-budget ladder. "
            "Budgets 3, 4, and 5 remain exactly fixed, so the remaining wall in that regime is no longer exposed by this literal-width increase."
        ),
    }
