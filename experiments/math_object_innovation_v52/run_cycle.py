...
def bundle_script(core_formula: str, patch_formula: str):
    ...
    families = set()
    if added:
        ops.append({"family": "ADD_BUNDLE", "literals": added})
        families.add("ADD_BUNDLE")
    if dropped:
        ops.append({"family": "DROP_BUNDLE", "literals": dropped})
        families.add("DROP_BUNDLE")
    if flipped:
        ops.append({"family": "FLIP_BUNDLE", "literals": flipped})
...
def build_report():
    ...
    for patch_formula in patch_formulas:
        ...
        for core_formula in core_formulas:
            families, ops = bundle_script(core_formula, patch_formula)
    ...
    return {
        ...
        "discovery_domain": (
            "semantic bundle-macro search over the shared exact core from v49, "
            "using ADD_BUNDLE, DROP_BUNDLE, and FLIP_BUNDLE as candidate families"
        ),
        "holdout_domain": "the five residual patch formulas from the v49 core-plus-patch frontier",
        "survivor": "bundle semantic-macro frontier",
        ...
        "best_total_macro_instances": total_ops,
        ...
        "strongest_claim": (
            ...
            "exactly scriptable using only two bundle-macro families, ADD_BUNDLE "
            "and FLIP_BUNDLE, and the best total macro-instance count drops from "
            "11 in v51 to 6."
        ),
    }
