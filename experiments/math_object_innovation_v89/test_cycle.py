...
def main() -> int:
    ...
    assert report["survivor"] == "lab-followup widened-certificate saturation boundary"
    ...
    assert [row["schema_gain_over_v88"] for row in ladder] == [0, 0, 0, 0, 0]
    assert [row["cost_gain_over_v88"] for row in ladder] == [0, 0, 0, 0, 0]
