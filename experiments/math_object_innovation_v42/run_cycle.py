...
def contiguous_partitions(values):
    ...
    for mask in range(1 << (len(values) - 1)):
        ...
        for index in range(len(values) - 1):
            if mask & (1 << index):
                partition.append(tuple(values[start:index + 1]))
                start = index + 1
        partition.append(tuple(values[start:]))
...
def build_report():
    ...
    feasible = []
    for partition in contiguous_partitions(NONTRIVIAL_SCORES):
    ...
    return {
        ...
        "discovery_domain": "contiguous score-abstraction search over exact mixed-sign witness regions on the hard v38 feature frontier",
        "holdout_domain": "the same 13 holdout fact states from v29 to v41",
        "survivor": "score-abstraction witness frontier",
        ...
        "all_feasible_partitions": [
            ...
            for total_cost, region_count, partition, _ in feasible
        ],
        "strongest_claim": (
            "In the searched contiguous score-abstraction space, the best exact mixed-sign witness partition is (7), (8), (9), (10,11), (12), "
            "which lowers total witness cost from 27 to 23. "
