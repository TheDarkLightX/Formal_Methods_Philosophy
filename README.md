...
This project is a set of tutorials about *how to think* in formal methods: what it means to model, to abstract, to verify, to learn from counterexamples, and to make tools “move symbols around” in a way that changes what you can confidently say.

The first tutorial uses a deck of cards to build an accurate mental picture of an **approximate state tracker**. We start inside a human’s head (a card counter), then zoom out to the abstract view (state machines, abstraction, symbolic tools, and counterexamples).

> Note: this is not gambling advice. The deck-of-cards setup is a convenient, familiar metaphor for *state*, *observation*, *abstraction*, and *evidence*.

## Table of contents

- [Part I: Inside the counter’s head](#part-i-inside-the-counters-head)
- [Part II: Zooming out. What is an approximate state tracker?](#part-ii-zooming-out-what-is-an-approximate-state-tracker)
- [Part III: From concrete states to symbolic states](#part-iii-from-concrete-states-to-symbolic-states)
- [Part IV: Counterexamples, black swans, and CEGIS](#part-iv-counterexamples-black-swans-and-cegis)
- [Part V: Safety, security, and two developer mindsets](#part-v-safety-security-and-two-developer-mindsets)
...
Think of it like keeping score in a game. You don’t remember every move in perfect detail;
you keep one or two numbers that you can update instantly.

### The counter’s internal state

Most counters don’t think “tuple.” They think “I’m keeping a running score, and I have a sense of how much shoe is left.”

- **Running score** (`running_count`): a whole number you keep in your head (…, `-2`, `-1`, `0`, `+1`, `+2`, …) and update every time you see a card.
...
#### How to visualize the running score (accurately)

If you want a picture you can *feel*:

- **A dial centered at 0.** Each revealed card nudges the needle: up for “low,” down for “high,” flat for “middle.” Over time it looks like a little staircase: `0, 1, 2, 1, 1, 0, ...`.
- **A balance scale of “seen” vs “unseen.”** Imagine every card comes with a tiny weight: `+1`, `0`, or `-1`. In common counting systems, the *total* weight of a fresh shoe is designed to be `0`. So if the weight you’ve already seen adds up to `running_count`, then the weight still hidden in the shoe must add up to `-running_count`.

That second picture is the key intuition: you can’t see the hidden cards directly, but you can track what you’ve removed, and because the total starts balanced, what remains is “tilted” the other way.

- When you see a “low” card, you add `+1` to `running_count`.
- When you see a “high” card, you add `-1` to `running_count`.
- When you see a “middle” card, you add `0`.
...
2. Classify it (low / high / middle) and apply the little `+1 / -1 / 0` rule you’ve memorized.
...
- `R` is the multiset of remaining cards (e.g., 6-deck shoe → many duplicates).
...
- **precision**: the abstraction doesn’t invent too many impossible behaviors (few false positives).
...
If this landing page worked, you should now have two synchronized pictures in your head:

1. **Inside view**: a human updating a tiny mental state from observations.
2. **Outside view**: a concrete system, an abstraction function, symbolic tools, and counterexamples as refuters.
